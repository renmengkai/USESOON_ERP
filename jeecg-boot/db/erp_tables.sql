...
CREATE TABLE `erp_order` (
  ...
  `node_code` varchar(2) NOT NULL COMMENT '订单节点：订货、定金、收款、交货、收货（节点字典）',
  ...
  `estimated_payback_date` datetime DEFAULT NULL COMMENT '预估回款日期',
  `actual_payback_date` datetime DEFAULT NULL COMMENT '实际回款日期',
  ...
  `total_sale_price` decimal(10,2) NOT NULL COMMENT '订单总售价',
  `total_cost_price` decimal(10,2) NOT NULL COMMENT '订单总成本',
  `total_margins` decimal(10,2) NOT NULL COMMENT '订单总利润',
  `profit_margin` decimal(10,2) NOT NULL COMMENT '订单利润率（%）',
  `is_valid` varchar(2) NOT NULL COMMENT '是否有效：0否1是',
  `tenant_id` varchar(32) NOT NULL COMMENT '租户ID',
  `opter` varchar(32) NOT NULL COMMENT '操作人编号',
  `opter_id` varchar(32) NOT NULL COMMENT '操作人姓名',
